			...
			<li className="flex flex-col gap-2 pt-3">
				...
				<ul className="flex flex-wrap gap-x-8 gap-y-2">
					{Children.toArray(exercise.answers.map((answer, ansI) => (
						<li className="text-base group">
							{isMobile ? (
								<label style={{ textDecorationThickness: "2px" }} className={twMerge("flex gap-2 group-hover:underline", (exam.sections[section].answers[exercise._id] === ansI + 1 ? "underline" : ""))} htmlFor={`question-${exercise._id}-${ansI}`}><span>{ansI + 1}</span> {answer}</label>
							) : (
