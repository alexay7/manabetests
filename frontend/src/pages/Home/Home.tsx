    ...
    return (
        <PageContainer>
            <div className="w-full flex font-semibold p-4 shadow-xl bg-white">
                <button className={`w-1/2 border-r p-3 rounded-full ${mode === "default" ? "bg-main-dark text-white" : "text-black bg-white"}`} onClick={()=>{
                    window.localStorage.setItem("mainmenu", "default");
                    setMode("default");
                }}
                >テスト
                </button>
                <button className={`w-1/2 border-r p-3 rounded-full ${mode === "custom" ? "bg-main-dark text-white" : "text-black bg-white"}`}  onClick={()=>{
